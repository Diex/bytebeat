...
const char string_0[] PROGMEM =  "t*(((t>>12)|(t>>8))&(63&(t>>4)));";
const char string_1[] PROGMEM =  "(((((t>>6|t<<1)+(t>>5|t<<3|t>>3)|t>>2|t<<1) & t>>12) ^ t>>16) & 255) >> 1 | ((t<<1&t>>9|t+1023>>9) & 255) >>1;";
const char string_2[] PROGMEM =  "char(char(t)<<(7&(t>>12)))+(t<<1)&t>>9|t+(t>>10)>>9;";
const char string_3[] PROGMEM =  "t^t%255;";
const char string_4[] PROGMEM =  "t ^ (char(t) + char(int(t) >> 8));";
const char string_5[] PROGMEM =  "t ^ (char(t) + char(int(t) >> 8)) | short(t) >> 6 | t >> 9;";
const char string_6[] PROGMEM =  "crowd();";
const char string_7[] PROGMEM =  "((t&4096)?((t*(t^t%255)|(t>>4))>>1):(t>>3)|((t&8192)?t<<2:t));";
const char string_8[] PROGMEM =  "((t&4096)?((t*(t^(char(t)+char(int(t)>>8)))|(t>>4))>>1):(t>>3)|((t&8192)?t<<2:t));";
const char string_9[] PROGMEM =  "255-((1L<<28)/(1+(t^0x5800)%0x8000) ^ t | t >> 4 | -t >> 10);";
const char string_10[] PROGMEM =  "//null;";
const char string_11[] PROGMEM =  "(t<<1 ^ (t + (t >> 8))) | t >> 2 | (char(t>>15^t>>16^0x75)>>(7&t>>10)&1?0:-1);";
const char string_12[] PROGMEM =  "t*2*(char((t>>10)^(t>>10)-2)%11) | t >> 2 | (char(t>>15^t>>16^0x75)>>(7&t>>10)&1?0:-1);";
const char string_13[] PROGMEM =  "(t*t>>(4-((t>>14)&7)))*t|(t&t-(2047&~(t>>7)))>>5|t>>3;";
const char string_14[] PROGMEM =  "8*t & 0x100 ? -8*t-1 : 8*t;";
const char string_15[] PROGMEM =  "t*(((t>>12)|(t>>8))&(63&(t>>4)));";
const char string_16[] PROGMEM =  "(t*5&t>>7)|(t*3&t>>10);";
const char string_17[] PROGMEM =  "(t>>6|t<<1)+(t>>5|t<<3|t>>3)|t>>2|t<<1;";
const char string_18[] PROGMEM =  "t ^ t >> 1;";
const char string_19[] PROGMEM =  "((t & 15) * (-t & 15) ^ !(t & 16) - 1) + 128;";
const char string_20[] PROGMEM =  "triangle_bells();";
const char string_21[] PROGMEM = "ft = t * (1 + (t >> 13 & 3 ^ t >> 18 & 3 ^ 1) << (t >> 15 & 1)), (((ft & 15) * (-ft & 15) ^ !(ft & 16) - 1)>> (t >> (7 + (t >> 13 & 3 ^ 1)) & 7)) + 128);";
const char string_22[] PROGMEM =  "(t & (t >> 7)) % 100 + (t  ^ (t >> 1 ) ) % 80; // + (t  | (t >> 113) ) % 75 + (t  ^ (t >> 81 ) ) % 80;";
const char string_23[] PROGMEM =  "(t << 31) * ( 1 + ( (t >> 11) & 7) ) * ((t>>13) % 13 * 31);";
const char string_24[] PROGMEM =  "t*27*(13&t>>10) % 128;";
const char string_25[] PROGMEM =  "(t * 9 *(39 & (t >> 13))) % 127;";
const char string_26[] PROGMEM =  "(t * 9 *(39 & (t >> 13))) % 127 | t * 27 * (38 & (t >> 12 )) % 64 ;         ";
const char string_27[] PROGMEM =  "(((t  * (t>>8 | t >> 5)) ) ^ 21 * (t>>1 | t >>2 | t>>3 | t>>5 | t >> 8 | t >> 13 | t >> 21) | 21  & 65 ) & (;>>11);";
...
volatile int dir = 0;
volatile long lastEncoder;
volatile boolean encoder = false;
...
void isr(){
  lastEncoder = millis();
  encoder = true;
  dir = digitalRead(3) == 0 ? 1 : -1; // leo el pin B del encoder  
}


void loop()
{
  if(encoder && millis() - lastEncoder > 60){    //debounce  

    encoder = false;
    formula = (formula + dir) < 0 ? qty - 1 : (formula + dir) % qty;    
    dir = 0;  
    ...
    TV.clear_screen();
    strcpy_P(formString, (char*)pgm_read_word(&(string_table[formula]))); // 
    TV.println(formString);
    Serial.println(formString);
    showTitle = true;    
  }

  if(!digitalRead(button))  { // pullup
    showTitle = false;
    TV.clear_screen();
  }
